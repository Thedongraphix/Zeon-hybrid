...
function createCustomAgent(llm, tools, memory, systemMessage) {
...
async function initializeAgentForUser(userId) {
        ...
        func: async ({ beneficiaryAddress, goalAmount, durationInSeconds, fundraiserName }) => {
            try {
                if (!isValidAddress(beneficiaryAddress))
                    return `❌ **Invalid Address:** The beneficiary address is not valid.`;
        ...
        func: async ({ contractAddress }) => {
            try {
                if (!isValidAddress(contractAddress))
                    return `❌ **Invalid Address:** The contract address is not valid.`;
                const provider = new ethers.JsonRpcProvider("https://sepolia.base.org");
                ...
                if (contributors.length === 0)
                    return `🤔 **No Contributions Yet.** [View Contract](${scanLink})`;
                const list = contributors.map((addr) => `- [\`${addr}\`](${generateBaseScanLink(addr, 'address')})`).join('\n');
...
async function initializeXmtpClient() {
    ...
    console.log(`🔥 XMTP client created for ${xmtp.address}`);
    return xmtp;
}
async function handleXmtpMessage(message, client) {
    const senderAddress = message.senderAddress;
    if (senderAddress.toLowerCase() === client.address.toLowerCase())
        return;
    ...
    try {
        ...
        const responseText = await processMessage(agent, config, message.content);
        ...
        try {
            await message.conversation.send("I encountered an error while processing your request. Please try again later.");
        }
        catch (sendError) {
...
async function startXmtpListener(client) {
